  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 space-y-8 pb-16">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">Trainer-verified milestones. Case metrics attach automatically — no need to re-type.</p>
      </div>
      ...
      <details className="rounded-xl border border-slate-200 bg-white text-sm shadow-sm open:shadow-md">
      ...
      <div className="space-y-10">
        {orderedLadders.map((ladder) => {
          ...
          return (
            <section key={ladder.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm space-y-3">
              ...
              <ol className="space-y-3">
                {ladder.steps.map((step) => {
                  ...
                  if (ach) {
                    return (
                      <li key={step.id} id={`competency-step-${step.id}`} className="rounded-lg border border-emerald-100 bg-emerald-50/20">
                  ...
                  if (locked) {
                    return (
                      <li
                        ...
                        className="rounded-lg border border-slate-100 bg-slate-50/50 px-3 py-2 text-xs text-slate-500"
                      >
                  ...
                  return (
                    <li
                      key={step.id}
                      id={`competency-step-${step.id}`}
                      className={`rounded-lg border p-3 border-amber-200/80 bg-white ${isFocus ? "ring-2 ring-amber-400 shadow-md" : ""}`}
                    >
      ...
      <section
        className={`rounded-xl border p-4 shadow-sm ${
          readinessSummary.readinessStatus === "ready" ? "border-emerald-200 bg-emerald-50/50" : "border-slate-200 bg-white"
        }`}
      >
