...
export async function POST(req: Request) {
  ...
  if (!latestReport) {
    return NextResponse.json({ ok: false, error: "No report found for case. Run seed-answers first." }, { status: 404 });
  }

  const summary = (latestReport.summary ?? {}) as Record<string, unknown>;
  const answers = (summary.answers ?? {}) as Record<string, Record<string, unknown>>;
  ...
  answers.consent_expectations.patient_preop_photo_set = {
    value: "Uploaded",
    evidence_ids: ["pre1", "pre2", "pre3", "pre4", "pre5", "pre6"],
    source: "manual",
  };
  ...
  answers.implantation_execution.patient_day0_recipient_images = {
    value: "Uploaded",
    evidence_ids: ["recipient_day0"],
    source: "manual",
  };
  ...
  const { error: upErr } = await supabase.from("reports").update({ summary: nextSummary }).eq("id", latestReport.id);
