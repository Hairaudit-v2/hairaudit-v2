...
import { updateTrainingCaseDetails } from "@/lib/academy/trainingCaseCorrections/service";
...
export async function PATCH(req: Request, ctx: { params: Promise<{ caseId: string }> }) {
  try {
    ...
    const result = await updateTrainingCaseDetails(supabase, caseId, access.userId, body);
