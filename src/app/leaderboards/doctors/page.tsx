...
export default async function DoctorsLeaderboardPage() {
  ...
  const caseRows = (cases ?? []).filter((c) => c?.id && c?.doctor_id);
  ...
  for (const c of caseRows as any[]) {
    const doctorId = String(c.doctor_id ?? "");
    ...
    const cur = aggByDoctor.get(doctorId) ?? {
      doctor_id: doctorId,
      eligible_cases: 0,
    ...
    for (const id of DOMAIN_ORDER) {
      ...
      const r0 = Number(d?.raw_score);
      if (Number.isFinite(w)) cur.domain_weighted_sum[id] = (cur.domain_weighted_sum[id] ?? 0) + w;
      if (Number.isFinite(r0)) cur.domain_raw_sum[id] = (cur.domain_raw_sum[id] ?? 0) + r0;
    }

    aggByDoctor.set(doctorId, cur);
  }

  const rows = Array.from(aggByDoctor.values())
    .map((a) => {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-10">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Doctor Leaderboard</h1>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.slice(0, 50).map((r, idx) => (
                <tr key={r.doctor_id} className={idx % 2 ? "bg-white" : "bg-slate-50/40"}>
                  <td className="px-4 py-3">{idx + 1}</td>
                  <td className="px-4 py-3 font-medium text-slate-900" title={r.doctor_id}>
                    {shortId(r.doctor_id)}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">{r.confidence_grade_mode} ({Number.isFinite(r.confidence_mult_avg) ? r.confidence_mult_avg.toFixed(2) : "—"})</td>
