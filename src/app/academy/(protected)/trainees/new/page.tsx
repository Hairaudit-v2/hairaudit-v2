...
export default async function NewTraineePage() {
  ...
  const { data: programs } = await supabase.from("training_programs").select("id, name").order("name");
  ...
  return (
    <div className="max-w-xl mx-auto px-4 sm:px-6 space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">Minimal fields — add assessment detail on cases.</p>
      </div>
